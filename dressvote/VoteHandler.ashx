...
public class VoteHandler : IHttpHandler {
    ...
    private void getVoteList(HttpContext context)
    {
        ...
        if (context.Request["currentPage"] != null && context.Request["currentPage"] != "null" && context.Request["pageSize"] != null && context.Request["pageSize"] != "null")
        {
            ...
            DataTable dt = Vote.getVoteListByPage(currentPage, pageSize);
            if (dt.Rows.Count > 0)
            {
    ...
    private void userVote(HttpContext context)
    {
        ...
        int result = 0;
        if (context.Request["openid"] != null && context.Request["clothingid"] != null && context.Request["openid"] != "null" && context.Request["openid"] != "-1")
        {
            if (!Vote.checkVote(context.Request["openid"].ToString()))
            {
                int clothingid = 0;
                int.TryParse(context.Request["clothingid"], out clothingid);
                if (clothingid > 0)
                {
                    string remark = "";
                    ...
                    result = Vote.voteClothing(context.Request["openid"].ToString(), nickname, clothingid, remark);
                }
            }
            else
            {
