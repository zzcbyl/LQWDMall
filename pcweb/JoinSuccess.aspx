<%@ Page Title="报名成功-卢勤问答平台官方书城" Language="C#" MasterPageFile="~/pcweb/Master.master" %>

<script runat="server">

</script>

<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
<div class="mainpage">
    <div style="background:#fff; margin:10px; padding:10px; line-height:22px;">
        <h3 style="text-align:center; margin-top:10px;">文明小使者畅游新加坡－2016新加坡精品冬令营</h3>

        <div style=" margin:20px 0; padding:10px; text-align:center;">
            <p style="text-align:center; color:Green; font-size:22px; font-weight:bold; line-height:35px;margin-top: 20px;"><i class="ico-ok-big"></i>报名成功</p>
            <div style="margin-top:50px;">
                您的报名信息我们已经收到，请您保持联系电话的畅通，有关夏令营事宜我们会通知您。
            </div>
            <div>您如果有任何问题请联系我们，联系方式：18601016361</div>
        </div>
    </div>
</div>
</asp:Content>

